...
export function handleLotteryOpen(event: LotteryOpen): void {
  ...
  lottery.ticketPrice = toBigDecimal(event.params.priceTicketInAlmanack);
...
export function handleTicketsPurchase(event: TicketsPurchase): void {
  ...
  if (user === null) {
    ...
    user.totalAlmanack = ZERO_BD;
    user.block = event.block.number;
    user.timestamp = event.block.timestamp;
    user.save();
  }
  user.totalTickets = user.totalTickets.plus(event.params.numberTickets);
  user.totalAlmanack = user.totalAlmanack.plus(event.params.numberTickets.toBigDecimal().times(lottery.ticketPrice));
  user.save();
...
export function handleTicketsClaim(event: TicketsClaim): void {
  ...
  user.totalAlmanack = user.totalAlmanack.plus(toBigDecimal(event.params.amount));
  user.save();
